...
// TestReadTextRefusesInvalidUTF8 drives the check through a connection, which is
// where section 8.1 asks for it: the read fails and the peer is told 1007.
func TestReadTextRefusesInvalidUTF8(t *testing.T) {
	var connBuf bytes.Buffer
	wc := newTestConn(nil, &connBuf, true)
	if err := wc.WriteMessage(BinaryMessage, []byte{0xc0, 0xaf}); err != nil {
		t.Fatalf("writing the payload returned %v", err)
	}
	// The opcode says text; the payload is an overlong form.
	frame := connBuf.Bytes()
	frame[0] = frame[0]&0xf0 | TextMessage

	var reply bytes.Buffer
	rc := newTestConn(bytes.NewReader(frame), &reply, false)
	if _, _, err := rc.ReadMessage(); err == nil {
		t.Fatal("a text message carrying an overlong form was accepted")
	}

	code := closeCodeOf(t, reply.Bytes())
	if code != CloseInvalidFramePayloadData {
		t.Fatalf("the peer was told %d, want %d", code, CloseInvalidFramePayloadData)
	}
}

// TestReadTextRefusesAMessageEndingMidRune is the case a per-byte check cannot
// see: every byte was legal and the message still stops halfway through a rune.
func TestReadTextRefusesAMessageEndingMidRune(t *testing.T) {
	var connBuf bytes.Buffer
	wc := newTestConn(nil, &connBuf, true)
	if err := wc.WriteMessage(TextMessage, []byte{0xf0, 0x9f}); err != nil {
		t.Fatalf("writing the payload returned %v", err)
	}

	var reply bytes.Buffer
	rc := newTestConn(bytes.NewReader(connBuf.Bytes()), &reply, false)
	if _, _, err := rc.ReadMessage(); err == nil {
		t.Fatal("a text message ending halfway through a rune was accepted")
	}
}

// TestReadTextAcceptsARuneSplitAcrossFrames is the other half, and it is why the
// check carries state instead of running once per frame.
func TestReadTextAcceptsARuneSplitAcrossFrames(t *testing.T) {
	var connBuf bytes.Buffer
	wc := newTestConn(nil, &connBuf, true)
	...
	for _, b := range []byte{0xf0, 0x9f, 0x9c, 0x82} {
		if _, err := w.Write([]byte{b}); err != nil {
			t.Fatalf("writing one byte returned %v", err)
		}
		// Flush each byte into a fragment of its own.
		if err := w.(*messageWriter).flushFrame(false, nil); err != nil {
			t.Fatalf("flushing a fragment returned %v", err)
		}
	}
	...
	rc := newTestConn(bytes.NewReader(connBuf.Bytes()), nil, false)
	kind, payload, err := rc.ReadMessage()
	if err != nil {
		t.Fatalf("a rune split one byte per fragment was refused: %v", err)
	}
	if kind != TextMessage || string(payload) != "\U0001f702" {
		t.Fatalf("read %d %q, want a text message carrying one rune", kind, payload)
	}
}

// TestReadBinaryIgnoresUTF8 keeps the check where the RFC puts it. A binary
// message is bytes, and refusing one for not being text would break every
// application that sends a JPEG.
func TestReadBinaryIgnoresUTF8(t *testing.T) {
	var connBuf bytes.Buffer
	wc := newTestConn(nil, &connBuf, true)
	payload := []byte{0xc0, 0xaf, 0xff, 0x00}
	if err := wc.WriteMessage(BinaryMessage, payload); err != nil {
		t.Fatalf("writing the payload returned %v", err)
	}

	rc := newTestConn(bytes.NewReader(connBuf.Bytes()), nil, false)
	kind, got, err := rc.ReadMessage()
	if err != nil {
		t.Fatalf("a binary message of arbitrary bytes was refused: %v", err)
	}
	if kind != BinaryMessage || !bytes.Equal(got, payload) {
		t.Fatalf("read %d %v, want %d %v", kind, got, BinaryMessage, payload)
	}
}

// closeCodeOf reads the close code out of the frames a connection wrote back.
func closeCodeOf(t *testing.T, frames []byte) int {
	t.Helper()

	// A writer, because reading a close frame answers it.
	c := newTestConn(bytes.NewReader(frames), io.Discard, true)
	_, _, err := c.ReadMessage()
	var closeErr *CloseError
	if !errors.As(err, &closeErr) {
		t.Fatalf("the connection wrote %v, want a close frame; read returned %v", frames, err)
	}
