...
// This file is the [joaju.Broker] implementation, and there is one of it.
//
...
// whole of it is a map from a name to a [joaju.Channel].
//
// The map is flat and keyed by [joaju.ChannelName.String],
...
// the inner key free to be [joaju.ChannelName.Requested], and then two customers
...
// restating as a discipline here. The one call this file makes into a [joaju.Channel]
// is a read, and [joaju.Channel] says the reads are not I/O.
//
...
// [joaju.SubscriptionPolicy], and it is for the tenant that owns the name.
//
// The [joaju.Observer]. [joaju.Observer.ChannelCreated] and [joaju.Observer.ChannelRemoved] are
// announced by whoever holds an Observer, which is the [joaju.Server].
...
// NewMemoryBroker is the [joaju.Broker] a server runs on: the channels this process
// holds, in a map.
//
...
// over [joaju.Relay], and the metrics routes ask the fleet rather than a shared store.
//
...
func NewMemoryBroker() joaju.Broker {
	return &memoryBroker{channels: make(map[string]joaju.Channel)}
}

// memoryBroker is the channels of one process, held under [joaju.ChannelName.String].
//
// It is safe for concurrent use, which [joaju.Broker] requires of an implementation:
...
type memoryBroker struct {
	mu       sync.RWMutex
	channels map[string]joaju.Channel
}

// Find is the channel held under the name, or [joaju.ErrNoChannel].
//
// The lookup is by [joaju.ChannelName.String] and never by [joaju.ChannelName.Requested].
...
// ([joaju.Server.fail] answers 404 with no detail), and the tenant is not news to the
// operator reading it.
func (b *memoryBroker) Find(_ context.Context, g auth.Grant, name joaju.ChannelName) (joaju.Channel, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: %s", joaju.ErrNoChannel, name.Requested())
	}
...
// a channel of this tenant, and [joaju.Channel.Subscribe] asks the
// [joaju.SubscriptionPolicy] again before anybody is seated.
func (b *memoryBroker) FindOrCreate(_ context.Context, g auth.Grant, name joaju.ChannelName) (joaju.Channel, error) {
	if err := b.accepts(g, name); err != nil {
		return nil, err
	}
...
// rather than believed from the caller. [joaju.Broker.Remove] says an implementation
...
//
// Asking is [joaju.Channel.Connections], the only thing the interface offers for it,
// and it is enough: it is a read, [joaju.Channel] says the reads of a channel are not
...
func (b *memoryBroker) Remove(_ context.Context, g auth.Grant, name joaju.ChannelName) error {
	if err := b.accepts(g, name); err != nil {
		return err
	}
...
//
// Whose a channel is comes from [joaju.ChannelName.Tenant], read off the channel's own
...
//
// The order is [joaju.ChannelName.String], for the reason [channel.Connections] is
...
// size of the process.
func (b *memoryBroker) All(_ context.Context, g auth.Grant) ([]joaju.Channel, error) {
	...
	slices.SortFunc(all, func(a, b joaju.Channel) int {
...
//
// The action is broadcasting.ChannelJoin, which is what a [joaju.SubscriptionPolicy]
...
func (b *memoryBroker) tenantOf(g auth.Grant) (string, error) {
	...
	if tenant == "" {
		return "", fmt.Errorf("%w: the grant carries no tenant", joaju.ErrNoGrant)
	}
...
//
// The tenant comparison is what [joaju.ErrWrongTenant] exists for, and it catches the
// one case [joaju.NewChannelName] cannot: a name built under one Grant and carried to
...
// channel should be. It is the refusal [NewChannel] and [joaju.Relay.Leave] make.
func (b *memoryBroker) accepts(g auth.Grant, name joaju.ChannelName) error {
	tenant, err := b.tenantOf(g)
	if err != nil {
		return err
	}
	...
	if tenant != name.Tenant() {
		return fmt.Errorf("%w: the grant is for %q and %s belongs to %q",
			joaju.ErrWrongTenant, tenant, name.Requested(), name.Tenant())
	}
